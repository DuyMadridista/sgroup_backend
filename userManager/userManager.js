...
//get total user by status
user_router.get('/:status', checkAccess("View user"), async (req, res) => {
    const status = req.params.status;
    console.log("duy dep trai " + status);
    try {
        const totalRecordsQuery = knex('user').where('status', status).count('id as totalCount').first();
        const totalRecordsResult = await totalRecordsQuery;
        console.log(`total records query result ${JSON.stringify(totalRecordsResult)}`);
        if (!totalRecordsResult || totalRecordsResult.totalCount === undefined) {
            return res.status(500).send("Error while getting total user count.");
        }
        const totalRecords = totalRecordsResult.totalCount;
        ...
        console.error(error);
        res.status(500).send("An error occurred. q111");
    }
});
//get total user by role
user_router.get('/:role', checkAccess("View user"), async (req, res) => {
    const role = req.params.role;
    try {
        const totalRecordsQuery = knex('user').where('role', role).count('id as totalCount').first();
        const totalRecordsResult = await totalRecordsQuery;
        console.log(`total records query result ${JSON.stringify(totalRecordsResult)}`);
        if (!totalRecordsResult || totalRecordsResult.totalCount === undefined) {
            return res.status(500).send("Error while getting total user count.");
